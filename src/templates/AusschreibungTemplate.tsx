...
interface AusschreibungTemplateProps { locale: Locale; }

export default function AusschreibungTemplate({ locale }: AusschreibungTemplateProps) {
  const daysAwayEvent = daysAway(EVENT_DATE);

  const mainDisciplinesM = locale === "de" ? ["100m", "400m Hürden", "800m", "1500m", "Diskuswurf (Mixed)"] : ["100m", "400m Hurdles", "800m", "1500m", "Discus Throw (Mixed)"];
  const mainDisciplinesW = locale === "de" ? ["100m", "400m", "400m Hürden", "800m", "1500m", "Hochsprung", "Weitsprung", "Kugelstoßen", "Diskuswurf (Mixed)"] : ["100m", "400m", "400m Hurdles", "800m", "1500m", "High Jump", "Long Jump", "Shot Put", "Discus Throw (Mixed)"];
  ...
  return (
    <ContentContainer>
      ...
      <div className="flex flex-col gap-2.5 text-center max-w-2xl mx-auto mb-8">
        <Title>{locale === "de" ? "Ausschreibung" : "Competition Rules & Info"}</Title>
        <p className="text-gray-300 text-base md:text-lg leading-relaxed">{locale === "de" ? "Offizielle Disziplinen, Prämien und Qualifikations-Standards für das Internationale Pfingstsportfest in Rehlingen." : "Official disciplines, prize money, and qualification standards for the International Pfingstsportfest in Rehlingen."}</p>{/*
          */}
      </div>
      ...
      <div className="flex flex-col gap-10 text-white">
        ...
        <div className="p-6 bg-white/5 border border-white/10 rounded-3xl backdrop-blur-xs flex flex-col md:flex-row items-center justify-between gap-6">
          <div className="flex flex-col gap-2">
            <span className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-xs font-black uppercase tracking-wider bg-tourLightOrange/10 border border-tourLightOrange/30 text-tourLightOrange max-w-fit">
              {locale === "de" ? "🏆 Silber Meeting Status" : "🏆 Silver Meeting Status"}
            </span>
            <p className="text-base text-gray-200 max-w-xl leading-relaxed mt-1">
              {locale === "de" ? <>Seit 2021 gehört das Rehlinger Pfingstsportfest stolz zur renommierten <strong className="text-white">World Athletics Continental Tour</strong> und wird weltweit als Silber Meeting der Leichtathletik gelistet.</> : <>Since 2021, the Rehlingen Pfingstsportfest is proudly part of the prestigious <strong className="text-white">World Athletics Continental Tour</strong> and is listed globally as a Silver Meeting.</>}
            </p>
          </div>
          <div className="flex flex-col items-center md:items-end justify-center py-2.5 px-6 bg-white/5 border border-white/5 rounded-2xl shrink-0 font-mono text-center md:text-right">
            <span className="text-xs text-gray-400 uppercase font-extrabold tracking-wider">{locale === "de" ? "Austragungsdatum" : "Event Date"}</span>
        ...
        <div className="flex flex-col gap-4">
          <h2 className="font-wa-headline text-2xl font-black text-tourLightOrange uppercase tracking-wide border-b border-white/5 pb-2.5">
            {locale === "de" ? "Zuständigkeiten & Kontakte" : "Responsibilities & Contacts"}
          </h2>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            ...
            <div className="p-6 bg-white/5 border border-white/10 rounded-2xl flex flex-col gap-3 backdrop-blur-xs">
              <div className="flex items-center gap-2">
                ...
                <h3 className="font-bold text-white text-lg">{locale === "de" ? "Athletenverpflichtung" : "Athlete Liaison"}</h3>
              </div>
              <p className="text-sm text-gray-300 leading-relaxed">
                {locale === "de" ? "Verpflichtung und Abstimmung für die internationalen Hauptwettbewerbe:" : "Engagement and coordination for international main competitions:"}
              </p>
            ...
            <div className="p-6 bg-white/5 border border-white/10 rounded-2xl flex flex-col gap-3 backdrop-blur-xs">
              <div className="flex items-center gap-2">
                ...
                <h3 className="font-bold text-white text-lg">{locale === "de" ? "Meeting-Direktion" : "Meeting Direction"}</h3>
              </div>
              <p className="text-sm text-gray-300 leading-relaxed">
                {locale === "de" ? "Leitung, Gesamtorganisation und wettkampf-technische Anfragen zum Sportfest:" : "Direction, general organization, and technical competition requests:"}
              </p>
              <div className="flex flex-col gap-1.5 text-base font-bold text-white mt-1">
                <span>Werner Klein</span>
                <span className="text-xs text-gray-400 font-extrabold uppercase bg-white/10 px-2 py-0.5 rounded-sm max-w-fit">{locale === "de" ? "Meeting-Direktor" : "Meeting Director"}</span>
            ...
            <div className="p-6 bg-white/5 border border-white/10 rounded-2xl flex flex-col gap-3 backdrop-blur-xs">
              <div className="flex items-center gap-2">
                <span className="text-xl">💼</span>
                <h3 className="font-bold text-white text-lg">{locale === "de" ? "Sponsoring & Marketing" : "Sponsoring & Marketing"}</h3>
              </div>
              <p className="text-sm text-gray-300 leading-relaxed">
                {locale === "de" ? "Kooperationen, Werbemaßnahmen, Sponsorenverträge und Mediapaket-Anfragen:" : "Cooperations, advertisements, sponsor contracts, and media requests:"}
              </p>
