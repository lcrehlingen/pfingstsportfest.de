...
import ResultsTable from "@/app/(de)/ergebnisse/ResultsTable";
...
interface ErgebnisseTemplateProps { locale: Locale; }

export default async function ErgebnisseTemplate({ locale }: ErgebnisseTemplateProps) {
  ...
  return (
    <ContentContainer>
      <div className="flex flex-col gap-2 text-center max-w-2xl mx-auto mb-4">
        <Title>{locale === 'de' ? 'Ergebnisse' : 'Results'}</Title>
        <p className="text-gray-400 text-sm md:text-base leading-relaxed">{locale === "de" ? "Verfolgen Sie die aktuellen Zeiten, Weiten und Höhen des Pfingstsportfests live oder stöbern Sie im Archiv aller PDF-Ergebnislisten seit dem Jahr 2000." : "Track current times, distances, and heights of the Pfingstsportfest live, or browse the archive of all PDF result lists since the year 2000."}</p>{/*
          */}
      </div>
